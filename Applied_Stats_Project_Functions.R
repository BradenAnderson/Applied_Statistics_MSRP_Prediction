...
############################################# Evaluate Best Subsets Search Model Finalists #################################################

build_lm_from_analysis_row <- function(train_data, row, response_var="log_MSRP", vif_matrix=FALSE){
  
  lm_formula <- build_formula_from_analysis_row(row=row, response_var=response_var)
  
  fit <- lm(lm_formula, data=train_data, x=vif_matrix)
...
build_formula_from_analysis_row <- function(row, response_var="log_MSRP"){
  
  predictors <- stringr::str_split(string=row[1, "predictors"],
                                     pattern="  ")[[1]]
  
  
  lm_formula <- as.formula(paste0(response_var, "~", stringr::str_c(predictors, collapse=" + ")))
  
  return(lm_formula)
  
}

final_model_comparisons <- function(train_data, model_search_df, response_var="log_MSRP"){
  
  set.seed(42)
  
  num_models <- nrow(model_search_df)
  
  for(row_index in 1:num_models){
    
    predictors <- stringr::str_split(string=model_search_df[row_index, "predictors"],
                                     pattern="  ")[[1]]
    
    # Create the formula to pass to caret for cross validation
    lm_formula <- build_formula_from_analysis_row(row=model_search_df[row_index,])
    
    # Specify the cross validation parameters for caret
    training_control <- caret::trainControl(method = "repeatedcv", number=10, repeats=20)

    # Perform the cross validation
    cv_fit <- caret::train(form = lm_formula, 
                           data=train_data,
                           method="lm",
                           trControl=training_control)
      
    
    # Save the cross validation statistics that caret calculated
    cv_rmse <- cv_fit$results[["RMSE"]]
    cv_rsquared <- cv_fit$results[["Rsquared"]]
    cv_mae <- cv_fit$results[["MAE"]]
    
    
    # Fit the same model, so we can calculate the leave-one-out and 
    # probability based selection statistics
    fit <- build_lm_from_strings(df=train_data,
                                 explanatory_vars=predictors,
                                 response_var=response_var,
                                 x=FALSE)
    
    # Calculate the probability based statistics
    bic_metric <- BIC(fit)
    aic_metric <- AIC(fit)
    # bic_olsrr_r <- olsrr::ols_sbc(fit, method="R") <-- redundant with the BIC call above
    bic_olsrr_sas <- olsrr::ols_sbc(fit, method="SAS")
    
    # Calculate the leave-one-out statistics
    pred_rsq <- olsrr::ols_pred_rsq(fit)
    press_stat <- olsrr::ols_press(fit)
    
    
    summary_df <- data.frame(predictor_count=length(predictors),
                             press=press_stat, 
                             pred_rsq=pred_rsq,
                             cv_rmse=cv_rmse,
                             ...
                             cv_mae=cv_mae,
                             bic=bic_metric,
                             bic_sas=bic_olsrr_sas,
                             aic=aic_metric,
                             predictors=stringr::str_c(predictors, collapse="  "))
  ...
  result_df <- calculate_relative_metric_pcts(result_df)
  
  return(result_df)
}


calculate_relative_metric_pcts <- function(df){
  
  
  # Calculate percent increases
  df[,"press_pct_increase"] <- ((df[,"press"] - min(df[,"press"]))/abs(min(df[,"press"])))*100
  df[,"cv_rmse_pct_increase"] <- ((df[,"cv_rmse"] - min(df[,"cv_rmse"]))/abs(min(df[,"cv_rmse"])))*100
  df[,"cv_mae_pct_increase"] <- ((df[,"cv_mae"] - min(df[,"cv_mae"]))/abs(min(df[,"cv_mae"])))*100
  df[,"bic_pct_increase"] <- ((df[,"bic"] - min(df[,"bic"]))/abs(min(df[,"bic"])))*100
  df[,"aic_pct_increase"] <- ((df[,"aic"] - min(df[,"aic"]))/abs(min(df[,"aic"])))*100
  
  df[,"cv_rsq_pct_decrease"] <- ((df[,"cv_rsquared"] - max(df[,"cv_rsquared"])) / max(df[,"cv_rsquared"])) * (-100)
  df[,"pred_rsq_pct_decrease"] <- ((df[,"pred_rsq"] - max(df[,"pred_rsq"])) / max(df[,"pred_rsq"])) * (-100)
...
############################################# End Evaluate Best Subsets Search Model Finalists #################################################





############################################# Adding Model Complexity (Objective 2) #################################################

get_all_complex_models <- function(initial_features, complex_features){
  
  all_potential_models <- c()
  
  complex_combos <- get_predictor_combos_manual(features=complex_features)
  
  for(index in 1:length(complex_combos)){
    
    this_combo <- complex_combos[[index]]
    
    this_model <- c(initial_features, this_combo)
    
    all_potential_models[[index]] <- this_model
    
  }
  
  return(all_potential_models)
}


cross_validate_complex_models <- function(train_data, initial_features, complex_features, response_var="log_MSRP"){
  
  set.seed(42)
  
  candidate_models <- get_all_complex_models(initial_features=initial_features, 
                                             complex_features=complex_features)
  
  
  # Add in the original model without additional complexity as a candidate for comparison.
  candidate_models[[length(candidate_models) + 1]] <- initial_features
  
  
  for(index in 1:length(candidate_models)){
    
    predictors <- candidate_models[[index]]
    ...
    model_performance_df <- data.frame(predictors=stringr::str_c(predictors,
                                                                 collapse="  "),
    ...
    if(index == 1){
      result_df <- model_performance_df
    }else{
      result_df <- rbind(result_df, model_performance_df)
    }
  }
...
############################################# END Adding Model Complexity (Objective 2) #################################################



############################################# lightGBM Functions #################################################

build_lgbm_dataset <- function(df, feature_columns, categorical_features, target_column="MSRP", test_data=FALSE, include_categoricals=FALSE){
...
get_test_set_preditions <- function(lgbm_model, test_dataset){
  
  # Make MSRP predictions on the test set
  predictions <- predict(object=lgbm_model, data=test_dataset$X_test)
  
  # Calculate the MSRP on the test set
  pred_rmse <- Metrics::rmse(actual=test_dataset$y_test,
...
lgbm_map_char_columns_to_int <- function(df, char_column_names){
  
  for(column_index in 1:length(char_column_names)){
    
    column_name <- char_column_names[column_index]
    
    df[,column_name] <- plyr::mapvalues(df[,column_name],
                                                        from=unique(df[,column_name]),
                                                        to=seq(0, (length(unique(df[,column_name]))-1)))
